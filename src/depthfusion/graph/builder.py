...
class DocumentEntityBuilder:
    ...
    def extract(
        ...
        """Extract entities from *chunk_text*, stamping each with *acl_allow*.
        ...
        source_file:
            Logical path of the source document (used in entity.source_files).
        acl_allow:
            Principal IDs permitted to see these entities. Propagated into
            ``entity.metadata["acl_allow"]`` (T-619 ACL inheritance). When
            *None*, the project slug is used as the sole allowed principal.

        Returns
        -------
        list[Entity]
            Extracted entities, each with ``metadata["acl_allow"]`` set.
            Returns empty list if *chunk_text* is blank.
        """
    ...
    # Alias kept for backward compat / convenience
    build = extract
