...
S-156 scope only: schema (principal_id, upn, display_name, groups, last_seen),
upsert_principal(), get_principal(), list_principals().  No device_id columns,
no role assignments, no authz hooks.  Those belong to S-157/S-158 and must NOT
be back-ported here.
...
class PrincipalStore:
    ...
    # ------------------------------------------------------------------
    # Public API (S-156 surface)
    # ------------------------------------------------------------------

    def upsert_principal(self, principal: Principal) -> None:
        """Insert or replace *principal*, updating ``last_seen`` to now.

        Called on every successful login to persist current group membership
        (S-156 AC-3).

        Parameters
        ----------
        principal:
            The principal to persist.  Only ``principal_id``, ``upn``,
            ``display_name``, and ``groups`` are stored; token fields are
            intentionally excluded (S-156 AC-4).
        """
        with self._lock:
            with closing(self._connect()) as conn:
                conn.execute(
    ...
    def list_principals(self, limit: int = 10) -> list[Principal]:
