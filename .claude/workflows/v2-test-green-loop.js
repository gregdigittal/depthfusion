...
while (attempt < maxAttempts) {
  ...
  const run = await agent(
    `In ${args.worktree} run, in order: (1) ruff check src/ ; (2) mypy src/ --ignore-missing-imports ; (3) python -m pytest tests/ -q --tb=line. Parse all failures into structured output (kind + one-line detail each, max 40; if more, keep the first 40 and note the count in summary). Empty failures array if all green.`,
  ...
  await parallel(chunk(run.failures, 5).map((group, gi) => () =>
    agent(
      `Fix these CI failures in ${args.worktree}. Minimal diffs only — no architectural changes, no test deletions, no assertion weakening. Failures:
