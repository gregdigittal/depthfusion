...
<sub>¹ **v2.0.0 vps-gpu CIQS projection is pending real-embedding validation.** The S-227 A/B benchmark ran BM25-only (no OpenAI embedding key); both profiles scored identically. E-71 (S-236) added `--profile` flag support to the harness and a paired Wilcoxon significance test — the infrastructure is ready. Re-run with `DEPTHFUSION_EMBEDDING_BACKEND=openai` once `OPENAI_API_KEY` is available to clear this gate. Until then, treat this row as *estimated*. See `docs/benchmarks/e71-profile-ab-2026-08-02.md`.</sub>
